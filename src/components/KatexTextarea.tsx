...
function KatexTextarea() {
    ...
    return (
        <div className="relative w-full h-full">
            {isRender ? (
                <>
                    <div className="bg-transparent p-4 text-sm leading-loose w-full h-full" dangerouslySetInnerHTML={{__html: renderedEquations}} ></div>
                </>
            ) : (
                <>
                    <textarea className="rounded-md bg-transparent p-4 text-md leading-loose w-full h-full border-0 outline-none" style={textAreaStyles}
                        value={content} onChange={e => dispatch(setContent(e.target.value))}
